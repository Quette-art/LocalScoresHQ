...
const GameDetails = ({
  ...
  const [graphicAction, setGraphicAction] = useState("");
  ...
  const getAgeGroup = (gameData) => {
    ...
    if (gameData.division) {
      return (
        gameData.division.split(" / ")[0] || gameData.division.split(" ")[0]
      );
    }
    return "Unknown";
  };
  ...
  const createScoreGraphic = async () => {
    ...
    const cardWidth = shareCardRef.current.offsetWidth || 360;
  ...
  const renderTeam = ({ teamName, score, winner, onClick, side }) => (
    <button
      type="button"
    ...
    >
      ...
      <span className="game-details-team-score">{isFinal ? score : "–"}</span>
    </button>
  );
    ...
    <div className="game-details-page">
      <div className="game-details-actions">
        ...
        <div className="game-details-share-actions">
          ...
          <button type="button" className="game-details-action" onClick={shareGame}>
            Share
            <span>↗</span>
          </button>
        </div>
