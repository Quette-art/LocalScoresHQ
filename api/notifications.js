...
const matchesEvent = (subscription, event) => {
  ...
  return keys.some((key) => {
    const settings = subscription.teamAlerts?.[key];
    if (!settings) return false;
    if (settings[alertType] === true) return true;

    // A saved score is currently treated as a final by the site. People who
    // selected Score Updates should still receive that final score alert.
    return event.type === "gameFinished" && settings.scoreUpdates === true;
  });
};
...
export default async function handler(request, response) {
  ...
  const result = await getMessaging(app).sendEachForMulticast({
    ...
    data: {
      ...
      body: copy.body,
      link: gameId
        ? `https://localscoreshq.com/game?id=${encodeURIComponent(gameId)}`
        : "https://localscoreshq.com/",
    },
    webpush: {
